...
export function pushNotify(text: string, action?: string, duration?: number): void {
  if (isTauri) {
    import('@tauri-apps/api/core').then(({ invoke }) => {
      invoke('push_notify', { text, action, duration }).catch((e) => {
        console.error('[notify] push_notify 调用失败:', e)
      })
    }).catch((e) => {
      console.error('[notify] 加载 core 失败:', e)
    })
    return
  }
