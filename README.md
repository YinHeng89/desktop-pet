...
| 状态码 | 触发条件                                                                               |
| ------ | -------------------------------------------------------------------------------------- |
| `400`  | body 不是合法 JSON、`text` 为空、或 `text` 超过 120 字（后两者返回 JSON `error` 字段） |
| `403`  | `Host` 头不是回环地址（`127.0.0.1` / `localhost` / `::1`）                             |
| `404`  | 非 `POST` 方法，或路径不是 `/notify`                                                   |
| `408`  | 单连接总耗时超过 10s（慢速攻击防护）                                                   |
| `413`  | 请求体超过 8KB，或 `Content-Length` 声明值超限                                         |
| `431`  | 请求头超过 8KB                                                                         |

### 安全边界

服务监听在 `127.0.0.1:8756`，**任何能访问该端口的程序都能让宠物说话**，因此做了两层防护：

- **`Host` 头白名单**：只接受回环地址挡住 DNS-rebinding 与浏览器跨站请求。
  任意网页都能对 `127.0.0.1` 发起 CORS 简单请求（`Content-Type: text/plain` 无预检），
  若不校验 Host，浏览恶意页面时宠物就会被随意操纵。
- **资源上限**：请求头 8KB、请求体 8KB、单连接总耗时 10s、并发连接 32。

> 端口被占用时，Rust 侧会 `emit("notify-server-error")`，而不是仅打印日志后静默失效。
